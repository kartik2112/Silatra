...
while True:
    ...
    handFound, hand, contours_of_hand, bboxHand = silatra_utils.get_my_hand(mask1)
    if handFound:
        mask2 = cv2.cvtColor(mask1,cv2.COLOR_GRAY2BGR)
        cv2.rectangle(mask2, (bboxHand[0], bboxHand[1]), (bboxHand[0]+bboxHand[2],bboxHand[1]+bboxHand[3]), (255, 0, 0), 2)
        cv2.putText(mask2,str("hand"),(int(bboxHand[0]-10),int(bboxHand[1]-20)),cv2.FONT_HERSHEY_SIMPLEX,0.8,(255,0,0),2,8);
        cv2.imwrite('../training-images/kartik/SampleImages/%d_MaskWFace.png'%(total_captured),mask2)
    ...
    if handFound:
        ...
        cv2.rectangle(mask2, (bboxHand[0], bboxHand[1]), (bboxHand[0]+bboxHand[2],bboxHand[1]+bboxHand[3]), (255, 0, 0), 2)
        cv2.putText(mask2,str("hand"),(int(bboxHand[0]-10),int(bboxHand[1]-20)),cv2.FONT_HERSHEY_SIMPLEX,0.8,(255,0,0),2,8);
        cv2.imwrite('../training-images/kartik/SampleImages/%d_MaskWOFace.png'%(total_captured),mask2)
