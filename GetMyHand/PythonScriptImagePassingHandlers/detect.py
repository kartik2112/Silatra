...
########    Imports moved to initializeEnvironmentFor_CV_Python() in skinColorSegmentation.cpp    #############
...
######    Moved inside predict_skin_pixels() function here  ###########################
...
def predict_skin_pixels(img_file, return_flag=True):
    model_data = ''
    with open('model.json') as model_file: model_data = model_file.read()
    model = model_from_json(model_data)
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
...
##### Only function was needed so commented this
